...
public class _14428 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<M;i++) {
            ...
            if(op.equals("1")) {
                ...
                int subIdx = query(tree, 1, 0, N-1, idx, idx2);
                //System.out.println(tree[subIdx]);
                System.out.println(query(tree, subIdx, tree[subIdx]) - N + 1);
            }
        }
    ...
    public static int query(int[] tree, int idx, int target) {
        if(idx >= tree.length / 2 && tree[idx] == target)
            return idx;
